...
template <size_t DOF>
class MasterMasterUDP : public barrett::systems::SingleIO<typename barrett::units::JointPositions<DOF>::type, typename barrett::units::JointPositions<DOF>::type> {
	BARRETT_UNITS_TEMPLATE_TYPEDEFS(DOF);

public:
	explicit MasterMasterUDP(barrett::systems::ExecutionManager* em, char* remoteHost, int port = 5554, const std::string& sysName = "MasterMasterUDP") :
	...
	virtual void operate() {

		{
			...
			while (true) {
				...
				else if (num_received>=7*sizeof(double)){
					numMissed = 0;
				}
				else
				{
...
private:
	DISALLOW_COPY_AND_ASSIGN(MasterMasterUDP);
};
